jQuery("#simulation")
  .on("mouseenter dragenter", ".s-c6904041-9e0a-4f0c-ac37-9489496715d8 .mouseenter", function(event, data) {
    ...
    if(jFirer.is("#s-Button_6") && jFirer.has(event.relatedTarget).length === 0) {
              ...
              "actions": [
                {
                  ...
                  "parameter": [ {
                    "target": [ "#s-c6904041-9e0a-4f0c-ac37-9489496715d8 #s-Button_6" ],
                    "attributes": {
      ...
      jEvent.launchCases(cases);
    } else if(jFirer.is("#s-Path_38") && jFirer.has(event.relatedTarget).length === 0) {
      event.backupState = true;
      event.target = jFirer;
      window.overTargets.push(jFirer.get(0));
      cases = [
  ...
  .on("mouseleave dragleave", ".s-c6904041-9e0a-4f0c-ac37-9489496715d8 .mouseleave", function(event, data) {
    var jEvent, jFirer, cases;
    if(jimUtil.isAlternateModeActive()) return;
    if(data === undefined) { data = event; }
    jEvent = jimEvent(event);
